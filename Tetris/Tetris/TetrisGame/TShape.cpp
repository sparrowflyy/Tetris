...
void TShape::update(float iTime)
{
	for (int i = 0; i < m_events.size(); i++) {
		if (m_events[i].type == GEvent::EventType::Motion) {
			GEventMotion* event = dynamic_cast<GEventMotion*>(&m_events[i]);
...
void TShape::initRectangles() {
	...
	for (int i = 0; i < TetrisShapes::shapeParts; i++) {
		m_rects.push_back(std::make_shared<sf::RectangleShape>(sf::Vector2f(TetrisShapes::rectSize, TetrisShapes::rectSize)));
